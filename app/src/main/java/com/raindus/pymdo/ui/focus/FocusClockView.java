...
public class FocusClockView extends View {
    ...
    public FocusClockView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        initPaint();
        initAnim();
    }
    ...
    private void initPaint() {
        ...
        mClockTextPaint.setTextSize(Utils.dipToPx(48));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mIsMusicPlay) {
            for (int i = 0; i < MUSIC_ANIM_NUM; i++) {
                if (!mMusicShow[i] || mMusicRadius[i] <= mClockRadius + Utils.dipToPx(2))
                    continue;
