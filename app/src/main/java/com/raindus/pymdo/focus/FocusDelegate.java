...
public class FocusDelegate implements View.OnClickListener {
    ...
    public FocusDelegate(Window window) {
        mBtnPauseOrSkip = window.findViewById(R.id.focus_clock_control);
        mBtnPauseOrSkip.setOnClickListener(this);

        mLlControlPanel = window.findViewById(R.id.focus_clock_control_layout);
        mBtnContinue = window.findViewById(R.id.focus_clock_continue);
        mBtnContinue.setOnClickListener(this);
        mBtnQuit = window.findViewById(R.id.focus_clock_quit);
    ...
    private void nextStatus() {
        if (mCurStatus == STATUS_TOMATO) {
            mTomato.bambooNum++;
            if (mTomato.bambooNum % mTomato.longRestIntervalNum == 0) {
            ...
            mCurTotalTime = mTomato.focusTime * 60;
    ...
    public void onStart() {
        mTomato = new FocusEntity(FocusParam.getTime(), FocusParam.getShortRest(),
                FocusParam.getLongRest(), FocusParam.getLongRestInterval());
        mCurStatus = STATUS_TOMATO;
        mCurTotalTime = mTomato.focusTime * 60;
        mTiming = 0;
    ...
    public void saveTomato() {
        ...
        if (mTomato != null && mTomato.bambooNum > 0) {
            mTomato.endTime = new Date().getTime();
            ObjectBox.FocusEntityBox.put(mTomato);
            FocusReportEntity.update(mTomato);
        }
    }
